// Локализация: RU (основной рынок) + EN + TR (у Яндекс Игр большая турецкая аудитория).
// Язык берётся из S.lang (выбор игрока), иначе из ysdk.environment.i18n.lang, иначе из браузера.
//
// ИМЕНА СУЩЕСТВ: в RU — свои стилизации, в EN/TR — латиница в «итальянском» духе,
// потому что мем-персонажей во всём мире знают именно так (Ballerino Cappuccino и т.д.).
// Один латинский набор служит и EN, и TR — турецкая аудитория знает мемы в латинице.
import type { Chain } from './config';

export type Lang = 'ru' | 'en' | 'tr';
export const LANGS: { code: Lang; label: string }[] = [
  { code: 'ru', label: 'Русский' }, { code: 'en', label: 'English' }, { code: 'tr', label: 'Türkçe' },
];

let lang: Lang = 'ru';
export const getLang = () => lang;
export const setLang = (l: Lang) => { lang = l; };

/** Код языка платформы/браузера → поддерживаемый язык. */
export function detectLang(sdkLang?: string | null): Lang {
  const raw = (sdkLang || navigator.language || 'ru').slice(0, 2).toLowerCase();
  return raw === 'en' ? 'en' : raw === 'tr' ? 'tr' : 'ru';
}

const RU: Record<string, string> = {
  'common.close': 'Закрыть',
  'common.claim': 'Забрать',
  'common.notEnoughCoins': 'Не хватает монет!',
  'common.notEnoughGems': 'Не хватает 💎',
  'common.notEnoughFunds': 'Не хватает средств',
  'common.boardFull': 'Поле заполнено!',
  'common.copied': 'Скопировано — кидай в чат!',
  'common.yes': 'Да',
  'common.cancel': 'Отмена',

  'hud.creature': 'Существо ({cost}🪙)',
  'hud.spawnFree': '🎁 Существо бесплатно!',
  'hud.freeIn': '🎁 бесплатно через {n} с',
  'hud.freeSoon': 'Не хватает монет — бесплатное через {n} с',
  'hud.incomeAd': '🎬 Доход ×{mult} (2 мин)',
  'hud.boostOn': 'Доход ×{mult}!',
  'hud.sec': 'с',
  'hud.min': 'м',
  'hud.day': 'д',
  'hud.shop': '💎 Магазин',
  'hud.quests': '📋 Задания',
  'hud.pedia': '📖 Мемпедия',
  'hud.arena': '🏆 Арена',
  // Короткие подписи под иконками нижней панели — места мало.
  'nav.shop': 'Магазин',
  'nav.quests': 'Задания',
  'nav.pedia': 'Мемпедия',
  'nav.settings': 'Ещё',
  'nav.arena': 'АРЕНА',
  'golden.tap': '✨ ЗОЛОТОЙ! +{n}🪙',

  'pedia.title': '📖 Мемпедия {found}/{total}',
  'pedia.discovered': '📖 Открыто: {name}! +{coins}🪙{gems}',
  'pedia.locked': 'Ещё не открыт',
  'pedia.secretHint': 'Секрет… ищи в сундуках 👀',

  'ftue.drag': '👆 Перетащи одно существо на другое!',
  'ftue.afterMerge': 'Отлично! Так растут уровни — а с ними доход',
  'ftue.income': 'Существа сами приносят монеты — следи за счётчиком 💰',
  'ftue.tap': 'Тапай существ на поле — они платят сразу!',
  'ftue.arena': 'Открой Арену — собери команду и бейся за кубки 🏆',
  'event.points': '{emoji} +{n} очков события',

  'row.title': '🔒 Дополнительный ряд',
  'row.desc': 'Больше места — больше существ\nи длиннее цепочки!',
  'row.buyCoins': 'Открыть за {n}🪙',
  'row.buyGems': 'Открыть за {n}💎',

  'streak.title': '📅 Ежедневный бонус',
  'streak.claimDay': 'Забрать бонус дня {n}!',
  'streak.lost': 'Серия из {n} дн. прервалась!',
  'streak.save': '🎬 Спасти серию за рекламу',
  'streak.restart': 'Начать серию заново',

  'quests.title': '📋 Задания дня',
  'quests.line': '{label}\n{prog}/{target} · награда {coins}🪙 +{gems}💎',
  'quests.footer': 'Новые задания — каждый день!',
  'card.level': 'уровень {n}',
  'card.income': '💰 Приносит {n}🪙 каждые 5 с',
  'card.type.melee': 'Ближний бой — бьёт первого в строю',
  'card.type.sniper': 'Снайпер — добивает самого раненого',
  'card.type.splash': 'По площади — задевает всех сразу',
  'card.toTeam': '🏆 В команду арены',
  'card.teamFull': 'В команде уже 5 бойцов',
  'card.sell': '💰 Продать за {n}🪙',
  'card.sellSure': '⚠️ Продать легендарку?',
  'card.sellLegend': '{name} собиралась долго.\nПродать за {n}🪙?',
  'league.wood': 'Деревяшка',
  'league.bronze': 'Бронза',
  'league.silver': 'Серебро',
  'league.gold': 'Золото',
  'league.platinum': 'Платина',
  'league.legend': 'ЛЕГЕНДА 67',
  'arena.leagueUp': '⬆️ Новая лига: {name}!',
  'arena.allClaimed': 'Все награды за кубки забраны.\nЖди новых порогов в обновлении!',
  'ftue.card': 'Зажми существо — характеристики, продажа и отправка в бой',
  'quest.wins': 'Победи в боях',
  'quest.merges': 'Слей существ',
  'quest.spawns': 'Заведи существ',
  'quest.taps': 'Покликай существ',

  'zones.title': '🗺️ Локации',
  'zones.chains': '{n} цепочки существ',
  'zones.unlock': 'Открыть: {coins}🪙 или {gems}💎',
  'zone.lab': 'Лаборатория',
  'zone.club': 'Неон-Клуб',
  'zone.watch': 'Ночной Дозор',
  'zone.space': 'Космо-База',

  'event.info': 'Осталось {days} дн. · Очки: {points} {emoji}\n\nСобытийные существа появляются при создании\nновых (шанс 25%). Сливай их и копи очки!',
  'event.footer': 'Когда событие закончится, его существа\nпревратятся в монеты — ничего не пропадёт!',
  'event.chest': '📦 сундук',
  'event.beach26': 'Пляжный сезон',
  'event.ny26': 'Новогодний движ',

  'arena.title': '🏆 Арена · {n}🏆',
  'arena.teamHint': 'Команда (бойцы не приносят доход на поле):',
  'arena.power': 'Сила команды: {n}',
  'arena.fight': '⚔️ В БОЙ!',
  'arena.needTeam': 'Сначала добавь бойцов!',
  'arena.atk': '⚔ Атака ур.{n}\n{cost}🪙',
  'arena.hp': '❤ Броня ур.{n}\n{cost}🪙',
  'arena.top': '🏅 Топ игроков',
  'arena.pickTitle': '➕ Выбери бойца с поля',
  'arena.pickEmpty': 'На поле пусто!\nКупи существ и возвращайся.',
  'arena.noRoom': 'На поле нет места!',
  'arena.win': '🎉 ПОБЕДА!',
  'arena.lose': '💀 Поражение…',
  'arena.winInfo': '{d}🏆  ·  +{coins}🪙',
  'arena.chestHint': '🎁 Каждая 3-я победа — сундук!',
  'arena.loseInfo': '{d}🏆\n\nПодкачай команду в казарме\nили слей существ повыше уровнем.',
  'arena.again': '⚔️ Ещё бой!',
  'arena.rematch': '🎬 Реванш с бустом ×{buff}',
  'arena.lbTitle': '🏅 Топ по кубкам',
  'arena.you': 'Ты',
  'arena.player': 'Игрок',
  'battle.vs': '⚔️ Ты  VS  {name}',
  'battle.speed': '▶ ×2',

  'wb.title': '⚔️ Битва недели',
  'wb.short': '⚔️ Битва недели',
  'wb.desc': '{a}  VS  {b}\n\nВыбери сторону — очки идут за слияния\nсуществ твоей команды. В конце недели —\nкристаллы по очкам. Сторону не сменить!',
  'wb.join': 'За {name}!',
  'wb.mine': '⭐ Твоя команда',
  'wb.points': 'Твои очки: {n} ⚔️',
  'wb.invite': '📣 Позвать друзей в мою команду',
  'wb.share': 'Я топлю за команду «{name}» в Битве недели Brainrot Lab: Merge ({n} очков) ⚔️ А ты за кого? Игра — на Яндекс Играх!',
  'wb.over': '⚔️ Битва недели окончена: +{gems}💎 за {points} очков!',

  'rename.title': '⭐ ЛЕГЕНДАРКА!',
  'rename.desc': 'Ты вырастил «{name}»!\nТакое существо заслуживает СОБСТВЕННОЕ имя.\nОно останется в твоей Мемпедии навсегда.',
  'rename.btn': '✏️ Дать имя',
  'rename.prompt': 'Имя для твоей легендарки:',
  'rename.done': 'Теперь это «{name}»!',
  'rename.brag': '📣 Похвастаться легендаркой',
  'rename.share': 'Моя легендарка «{name}» уже качает в Brainrot Lab: Merge 🏆 Покажи свою! Игра — на Яндекс Играх.',

  's67.title': '⁉️ 6 7 !!!',
  's67.desc': 'Ты выбил СЕКРЕТНОГО 67!\nШанс — всего 6,7%.\n\nТаких игроков — единицы.\nСольёшь до ЛЕГЕНДО 67 — станешь легендой сам.',
  's67.brag': '📣 Скопировать хвастовство',
  's67.share': 'Я выбил секретного 67 в Brainrot Lab: Merge — шанс всего 6,7% 🔵6️⃣7️⃣ Слабо повторить? Ищи игру на Яндекс Играх!',

  'starter.title': '🎁 Подарок новичку',
  'starter.desc': 'Стартовый набор — выгода ×5\n\n150💎 + 5000🪙\n+ 7 дней без рекламы\n\nТолько один раз!',
  'starter.btn': 'Забрать со скидкой',

  'offline.title': '💤 Пока вас не было…',
  'offline.desc': 'Твои брейнроты наработали:\n🪙 {n}',
  'offline.claim': 'Забрать {n}🪙',
  'offline.claim2': '🎬 Забрать ×2 ({n}🪙)',
  'offline.tier': 'Склад копит {hours} ч и платит {percent}% ставки',
  'offline.upsell': '📦 Расширить склад до {hours} ч и 100%',
  'p.offline_vip.title': 'Склад брейнротов',
  'p.offline_vip.desc': 'Офлайн копит 5 часов по полной ставке (вместо 2 часов и половины) — навсегда',

  'shop.title': '💎 Магазин',
  'shop.freeChest': '🎁 Бесплатный сундук за рекламу',
  'shop.freeChestWait': '🎁 Бесплатный сундук через {n} мин',
  'shop.chest': '📦 Сундук существа — {n}💎',
  'shop.failed': 'Покупка не прошла',
  'shop.boost': '⚡ Буст дохода ×{mult} (10 мин) — {gems}💎',
  'p.starter.title': 'Стартовый набор — выгода ×5',
  'p.starter.desc': '150💎 + 5000🪙 + 7 дней без рекламы (только 1 раз)',
  'p.gems_s.title': 'Горсть кристаллов',
  'p.gems_s.desc': '80💎',
  'p.gems_m.title': 'Мешок кристаллов',
  'p.gems_m.desc': '500💎 · выгода +25%',
  'p.gems_l.title': 'Сундук кристаллов',
  'p.gems_l.desc': '1200💎 · выгода +50%',
  'p.no_ads.title': 'Отключить рекламу',
  'p.no_ads.desc': 'Убирает всю принудительную рекламу навсегда',
  'chest.secret': '⁉️ ВЫПАЛ СЕКРЕТНЫЙ 67!!!',
  'chest.secretFull': 'Поле забито — +67💎 (это знак)',
  'chest.new': 'Новое существо из сундука!',
  'chest.full': 'Поле забито — +{n}🪙',

  'rarity.common': 'обычный',
  'rarity.rare': 'редкий',
  'rarity.epic': 'эпический',
  'rarity.legendary': 'легендарный',

  'set.title': '⚙️ Настройки',
  'set.soundOn': '🔊 Звук: вкл',
  'set.soundOff': '🔇 Звук: выкл',
  'set.lang': '🌐 Язык',
  'set.reset': '🗑️ Сбросить прогресс',
  'set.resetAsk': 'Точно удалить ВЕСЬ прогресс?\nОтменить будет нельзя.',
  'set.resetYes': 'Да, удалить всё',
  'set.version': 'Brainrot Lab: Merge · v{v}',
  'set.credits': 'Сделано для Яндекс Игр',
  'set.hapticsOn': '📳 Вибрация: вкл',
  'set.hapticsOff': '📴 Вибрация: выкл',
  'set.shortcut': '📌 Добавить ярлык на экран',
  'set.promo': '🎟️ Ввести промокод',
  'promo.prompt': 'Введи промокод (ищи их в соцсетях игры):',
  'promo.ok': '🎟️ Код принят!{gems}{egg}',
  'promo.gems': ' +{n}💎',
  'promo.egg': ' + {name}',
  'promo.used': 'Этот код уже использован',
  'promo.bad': 'Код не найден или истёк',

  'ach.title': '🏅 Награды {done}/{total}',
  'ach.tab': '🏅 Награды',
  'ach.tabPedia': '📖 Существа',
  'ach.claimed': 'Забрано',
  'ach.merges': 'Слияний: {n}',
  'ach.taps': 'Тапов: {n}',
  'ach.spawns': 'Существ заведено: {n}',
  'ach.golden': 'Золотых поймано: {n}',
  'ach.wins': 'Побед на арене: {n}',
  'ach.cups': 'Кубков набрано: {n}',
  'ach.eggs': 'Яиц вылуплено: {n}',
  'ach.sold': 'Продано «друзей»: {n}',
  'ach.legends': 'Легендарок выращено: {n}',
  'ach.pedia': 'Открыто в Мемпедии: {n}',
  'ach.zones': 'Локаций открыто: {n}',
  'ach.secret': '6️⃣7️⃣ НАВСЕГДА',
  'ach.spaghetti': '🍝 Спагетти-магнат',
  'ach.share': '📣 Похвастаться',
  'ach.shareSold': 'Я продал {n} «друзей» в Brainrot Lab: Merge 💰 Бизнес есть бизнес. Игра — на Яндекс Играх!',
  'ach.shareSecret': 'У меня есть секретный 67 в Brainrot Lab: Merge 🔵6️⃣7️⃣ Шанс — 6,7%. Слабо повторить?',
  'ach.shareSpaghetti': 'Я вырастил Спагеттиссимо Прайм в Brainrot Lab: Merge 🍝 Спагетти-магнат — это я. Игра на Яндекс Играх!',

  'pass.title': '📔 Журнал · тир {tier}/{total}',
  'pass.tab': '📔 Журнал',
  'pass.tabQuests': '📋 Задания дня',
  'pass.progress': '{points}/{need} очков до следующего тира',
  'pass.season': 'Сезон {id} · до конца {days} дн.',
  'pass.how': 'Очки: слияние +{merge}, победа +{win},\nквест +{quest}, золотой +{golden}',
  'pass.tier': 'Тир {n}',
  'pass.buy': '🧪 Премиум-трек — {gems}💎',
  'pass.bought': '🧪 Премиум активен: награды ×2',
  'pass.premiumOnly': 'только премиум',
  'pass.locked': 'Ещё {n} очков',
  'pass.coinsMin': '{n} мин дохода',
  'pass.done': 'Трек закрыт! Новый — с новым сезоном',
  'pass.buyAsk': 'Премиум-трек сезона {id}\n\n• все награды трека ×2\n• рамка «Лаборант» на финале\n\nСписать {gems}💎?',
  'pass.gained': '📔 +{n}',
  'frame.pass': 'Лаборант',

  'myth.title': '🧪 Мифики {found}/{total}',
  'myth.tab': '🧪 Мифики',
  'myth.fuse': '🧪 Слить с легендаркой',
  'myth.pickTitle': '🧪 Выбери вторую легендарку',
  'myth.pickEmpty': 'Нужна вторая легендарка на поле.\nМифик получается из ДВУХ разных.',
  'myth.nope': 'Эта пара ничего не даёт — существа целы',
  'myth.born': '🧪 МИФИК!',
  'myth.desc': 'Из двух легендарок вышло существо,\nкакого нет ни у кого рядом.\nДоход — как у трёх с половиной легендарок.',
  'myth.brag': '📣 Похвастаться мификом',
  'myth.share': 'Я собрал мифика «{name}» в Brainrot Lab: Merge 🧪 Два легендарных в одном. Игра — на Яндекс Играх!',
  'myth.locked': 'Рецепт не открыт',
  'myth.hint': 'Слей этих двух легендарок',
  'myth.crocaccino': 'Крокаччино Гранде',
  'myth.tralaccino': 'Тралаччино Прима',
  'myth.discotambo': 'Диско-Тамбо Мяуссимо',
  'myth.roboletto': 'Роболетто Фруттиссимо',
  'myth.guardiano67': 'Гвардиано 67',
  'myth.cosmocapy': 'Космо-Капибарро НЛОниссимо',
  'myth.pastalien': 'Пастальено Галактико',
  'myth.sixseventissimo': 'Шестьсемьтиссимо Паста',

  'pact.btn': '🤝 Код друга',
  'pact.title': '🤝 Пакт с другом',
  'pact.mine': 'Твой код',
  'pact.mineHint': 'Раздай код друзьям: награду получает тот,\nкто его ввёл. Чтобы получить свою —\nвведи код друга.',
  'pact.copy': '📣 Скопировать приглашение',
  'pact.share': 'Играю в Brainrot Lab: Merge 🧪 Введи мой код {code} — получишь {gems}💎 и редкое яйцо. Игра на Яндекс Играх!',
  'pact.enter': '🤝 Ввести код друга',
  'pact.prompt': 'Код друга — 4 буквы и 2 цифры:',
  'pact.namePrompt': 'Как зовут друга? (можно пропустить)',
  'pact.done': '🤝 Пакт заключён!',
  'pact.reward': '+{gems}💎, редкое яйцо\nи +{pct}% дохода навсегда',
  'pact.already': 'Пакт уже заключён — он один на игрока',
  'pact.self': 'Это твой собственный код 🙂',
  'pact.bad': 'Такого кода нет — проверь буквы и цифры',
  'pact.active': '🤝 Пакт с {name}',
  'pact.bonus': 'Доход +{pct}% навсегда',
  'pact.partner': 'другом',
  'pact.honest': 'Без сервера игра не знает, что делает друг,\nпоэтому и не показывает выдуманных цифр:\nздесь только твоя награда и твой бонус.',

  'pres.btn': '🔄 Новая лаборатория',
  'pres.title': '🔄 Новая лаборатория',
  'pres.neurons': '🧠 Нейроны: {n}',
  'pres.gain': 'За переезд получишь: +{n} 🧠',
  'pres.locked': 'Переезд откроется, когда все локации\nоткрыты, а Космо-База заполнена\nсуществами высшего уровня.',
  'pres.move': '🚚 Переехать ({n} 🧠)',
  'pres.count': 'Переездов: {n}',
  'pres.askTitle': '🚚 Точно переезжаем?',
  'pres.keep': 'СОХРАНИТСЯ:\n💎 кристаллы и покупки\n📖 Мемпедия и имена легендарок\n🏆 кубки, лиги, сезоны, рамки\n🏅 награды и журнал\n🥚 яйцо в инкубаторе\n⚔️ КОМАНДА АРЕНЫ и казарма',
  'pres.lose': 'ОБНУЛИТСЯ:\n🪙 монеты\n🧪 все поля и локации (кроме Лаборатории)\n🎁 цена существ (начнётся заново)',
  'pres.yes': '🚚 Переезжаем!',
  'pres.done': '🔄 Новая лаборатория готова! +{n} 🧠',
  'perk.cooldown': '⏱️ Быстрые пробирки',
  'perk.cooldownDesc': 'Бесплатное существо на {n} с быстрее',
  'perk.quality': '🧫 Качество пробирок',
  'perk.qualityDesc': '{n}% существ приходят 2-го уровня',
  'perk.lucky': '✨ Удачное слияние',
  'perk.luckyDesc': '{n}% слияний дают сразу +2 уровня',
  'perk.income': '💰 Доход',
  'perk.incomeDesc': 'Доход поля +{n}%',
  'perk.offline': '💤 Склад',
  'perk.offlineDesc': 'Офлайн копит на {n} мин дольше',
  'perk.combo': '👆 Комбо',
  'perk.comboDesc': 'Максимум тап-комбо ×{n}',
  'perk.buy': '{n} 🧠',
  'perk.max': 'макс.',

  'skin.title': '🎨 Оформление поля',
  'skin.btn': '🎨 Оформление поля',
  'skin.buy': 'Купить — {gems}💎',
  'skin.on': '✅ Надет',
  'skin.use': 'Надеть',
  'skin.earn': '👹 За 3 побеждённых босса',
  'skin.default': 'Лаборатория',
  'skin.neon': 'Неон',
  'skin.candy': 'Карамель',
  'skin.gold': 'Золото',
  'skin.lab': 'Био-лаб',

  'tour.chip': '🏟️ Турнир выходных: {n} побед',
  'tour.title': '🏟️ Турнир выходных',
  'tour.desc': 'Победы на арене в субботу и воскресенье\nидут в зачёт турнира. Твой счёт: {n}',
  'tour.line': '{wins} побед — {reward}',
  'tour.none': 'Турнир идёт только в выходные.\nЗаходи в субботу и воскресенье!',
  'tour.coinsMin': '{n} мин дохода',

  'boss.btn': '👹 Босс лиги: {name}',
  'boss.done': '👹 Босс лиги побеждён ✅',
  'boss.locked': '👹 Босс откроется в лиге {name}',
  'boss.title': '👹 {name}',
  'boss.desc': 'Один против твоей пятёрки.\nHP {hp} · урон {dmg} раз в {spd}с · {type}',
  'boss.hint': 'Сплэш-бойцы бьют одиночную цель\nв 0.45 силы — против босса берите\nбойцов по одной цели и качайте казарму.',
  'boss.fight': '⚔️ Вызвать босса',
  'boss.win': '🏆 БОСС ПОВЕРЖЕН!',
  'boss.reward': 'Награда: +{gems}💎 и золотое яйцо',
  'boss.lose': '💀 Босс оказался сильнее',
  'boss.loseInfo': 'Босс — проверка на силу, а не удача.\nПодкачай казарму, слей существ повыше\nили замени сплэш-бойцов — и возвращайся.',
  'boss.rematch': '🎬 Реванш с бустом ×{buff}',
  'ach.bosses': 'Боссов побеждено: {n}',

  'season.title': '🗓️ Итоги сезона',
  'season.peak': 'Твоя лучшая лига сезона:',
  'season.reward': 'Награда: +{gems}💎',
  'season.frame': '🖼️ Открыта рамка профиля «{name}»!',
  'season.reset': 'Кубки нового сезона: {cups}🏆\n(пороги наград сохранены — они\nсчитаются по личному рекорду)',
  'season.claim': 'Забрать и начать сезон',
  'season.now': 'Сезон {id} · до конца {days} дн.',
  'season.frameHint': 'Тап — сменить рамку',

  'mut.banner': '🧬 Сегодня мутирует {name} — доход ×{mult}!',
  'mut.card': '🧬 Мутация дня: доход ×{mult}',
  'streak.tomorrow': 'Завтра: {reward}',
  'streak.comeBack': 'Возвращайся завтра за {reward}!',

  'egg.title': '🥚 Инкубатор',
  'egg.chipNone': '🥚 пусто',
  'egg.chipReady': '🥚 готово!',
  'egg.none': 'Инкубатор пуст.\n\nЯйца дают:\n🏆 каждая {n}-я победа на арене — обычное\n⬆️ новая лига — редкое\n🎉 награды событий — золотое',
  'egg.hatchIn': 'Вылупится через {time}',
  'egg.ready': 'Яйцо готово вылупиться!',
  'egg.hatch': '🐣 Вылупить!',
  'egg.ad': '🎬 Ускорить на {n} мин',
  'egg.adCap': '🎬 Реклама: лимит на сегодня',
  'egg.gem': '💎 −{min} мин за {cost}💎',
  'egg.queue': 'В очереди ещё {n} 🥚',
  'egg.got': '🥚 Получено {name}!',
  'egg.gotQueued': '🥚 {name} — в очереди инкубатора',
  'egg.hatched': '🐣 Из яйца вылупился {name}!',
  'egg.hatchedFull': 'Поле забито — вместо существа +{n}🪙',
  'egg.gems': '+{n}💎 из яйца',
  'egg.common': 'обычное яйцо',
  'egg.rare': 'редкое яйцо',
  'egg.gold': 'золотое яйцо',
  'egg.reward': 'Внутри: существо {lvl} ур.{gems}',
  'egg.rewardGems': ' + {n}💎',

  'ask.reviewTitle': '⭐ Нравится игра?',
  'ask.reviewDesc': 'Оценка помогает игре попасть\nв подборки Яндекс Игр —\nи мы делаем больше существ!',
  'ask.reviewYes': '⭐ Оценить игру',
  'ask.later': 'Не сейчас',
  'ask.reviewDone': 'Спасибо! Это правда помогает 💛',
  'ask.shortcutTitle': '📌 Ярлык на рабочий стол',
  'ask.shortcutDesc': 'Добавь иконку на экран телефона —\nбудешь заходить в один тап,\nа не искать игру каждый раз.',
  'ask.shortcutYes': '📌 Добавить ярлык',
  'ask.shortcutDone': 'Готово! Ищи иконку на экране 📱',
  'ask.authTitle': '☁️ Не потеряй прогресс',
  'ask.authDesc': 'Сейчас прогресс живёт только\nв этом браузере. Войди в аккаунт —\nи он сохранится в облаке.',
  'ask.authYes': '☁️ Войти в аккаунт',
  'ask.authDone': 'Готово — прогресс в облаке ☁️',
};

const EN: Record<string, string> = {
  'common.close': 'Close',
  'common.claim': 'Claim',
  'common.notEnoughCoins': 'Not enough coins!',
  'common.notEnoughGems': 'Not enough 💎',
  'common.notEnoughFunds': 'Not enough funds',
  'common.boardFull': 'Board is full!',
  'common.copied': 'Copied — paste it in chat!',
  'common.yes': 'Yes',
  'common.cancel': 'Cancel',

  'hud.creature': 'Creature ({cost}🪙)',
  'hud.spawnFree': '🎁 Free creature!',
  'hud.freeIn': '🎁 free in {n}s',
  'hud.freeSoon': 'Not enough coins — free one in {n}s',
  'hud.incomeAd': '🎬 Income ×{mult} (2 min)',
  'hud.boostOn': 'Income ×{mult}!',
  'hud.sec': 's',
  'hud.min': 'm',
  'hud.day': 'd',
  'hud.shop': '💎 Shop',
  'hud.quests': '📋 Quests',
  'hud.pedia': '📖 Mempedia',
  'hud.arena': '🏆 Arena',
  'nav.shop': 'Shop',
  'nav.quests': 'Quests',
  'nav.pedia': 'Mempedia',
  'nav.settings': 'More',
  'nav.arena': 'ARENA',
  'golden.tap': '✨ GOLDEN! +{n}🪙',

  'pedia.title': '📖 Mempedia {found}/{total}',
  'pedia.discovered': '📖 Unlocked: {name}! +{coins}🪙{gems}',
  'pedia.locked': 'Not found yet',
  'pedia.secretHint': 'A secret… look inside chests 👀',

  'ftue.drag': '👆 Drag one creature onto another!',
  'ftue.afterMerge': 'Nice! That is how levels grow — and income with them',
  'ftue.income': 'Creatures earn coins on their own — watch the counter 💰',
  'ftue.tap': 'Tap creatures on the board — they pay instantly!',
  'ftue.arena': 'Open the Arena — build a team and fight for trophies 🏆',
  'event.points': '{emoji} +{n} event points',

  'row.title': '🔒 Extra row',
  'row.desc': 'More space means more creatures\nand longer chains!',
  'row.buyCoins': 'Unlock for {n}🪙',
  'row.buyGems': 'Unlock for {n}💎',

  'streak.title': '📅 Daily bonus',
  'streak.claimDay': 'Claim day {n} bonus!',
  'streak.lost': 'Your {n}-day streak broke!',
  'streak.save': '🎬 Save streak with an ad',
  'streak.restart': 'Start a new streak',

  'quests.title': '📋 Daily quests',
  'quests.line': '{label}\n{prog}/{target} · reward {coins}🪙 +{gems}💎',
  'quests.footer': 'New quests every day!',
  'card.level': 'level {n}',
  'card.income': '💰 Earns {n}🪙 every 5s',
  'card.type.melee': 'Melee — hits the front of the line',
  'card.type.sniper': 'Sniper — finishes off the weakest',
  'card.type.splash': 'Splash — hits everyone at once',
  'card.toTeam': '🏆 Add to arena team',
  'card.teamFull': 'The team already has 5 fighters',
  'card.sell': '💰 Sell for {n}🪙',
  'card.sellSure': '⚠️ Sell your legendary?',
  'card.sellLegend': '{name} took a long time to build.\nSell for {n}🪙?',
  'league.wood': 'Wood',
  'league.bronze': 'Bronze',
  'league.silver': 'Silver',
  'league.gold': 'Gold',
  'league.platinum': 'Platinum',
  'league.legend': 'LEGEND 67',
  'arena.leagueUp': '⬆️ New league: {name}!',
  'arena.allClaimed': 'Every trophy reward is claimed.\nMore thresholds are coming in an update!',
  'ftue.card': 'Hold a creature — stats, selling and sending it to battle',
  'quest.wins': 'Win arena battles',
  'quest.merges': 'Merge creatures',
  'quest.spawns': 'Get new creatures',
  'quest.taps': 'Tap your creatures',

  'zones.title': '🗺️ Locations',
  'zones.chains': '{n} creature chains',
  'zones.unlock': 'Unlock: {coins}🪙 or {gems}💎',
  'zone.lab': 'The Lab',
  'zone.club': 'Neon Club',
  'zone.watch': 'Night Watch',
  'zone.space': 'Cosmo Base',

  'event.info': '{days} days left · Points: {points} {emoji}\n\nEvent creatures appear when you buy\nnew ones (25% chance). Merge them for points!',
  'event.footer': 'When the event ends its creatures\nturn into coins — nothing is lost!',
  'event.chest': '📦 chest',
  'event.beach26': 'Beach Season',
  'event.ny26': 'New Year Party',

  'arena.title': '🏆 Arena · {n}🏆',
  'arena.teamHint': 'Team (fighters earn no income on the board):',
  'arena.power': 'Team power: {n}',
  'arena.fight': '⚔️ FIGHT!',
  'arena.needTeam': 'Add some fighters first!',
  'arena.atk': '⚔ Attack lv.{n}\n{cost}🪙',
  'arena.hp': '❤ Armor lv.{n}\n{cost}🪙',
  'arena.top': '🏅 Top players',
  'arena.pickTitle': '➕ Pick a fighter from the board',
  'arena.pickEmpty': 'The board is empty!\nGet some creatures and come back.',
  'arena.noRoom': 'No free cell on the board!',
  'arena.win': '🎉 VICTORY!',
  'arena.lose': '💀 Defeat…',
  'arena.winInfo': '{d}🏆  ·  +{coins}🪙',
  'arena.chestHint': '🎁 Every 3rd win gives a chest!',
  'arena.loseInfo': '{d}🏆\n\nUpgrade your team in the barracks\nor merge higher-level creatures.',
  'arena.again': '⚔️ One more fight!',
  'arena.rematch': '🎬 Rematch with ×{buff} buff',
  'arena.lbTitle': '🏅 Trophy leaderboard',
  'arena.you': 'You',
  'arena.player': 'Player',
  'battle.vs': '⚔️ You  VS  {name}',
  'battle.speed': '▶ ×2',

  'wb.title': '⚔️ Battle of the Week',
  'wb.short': '⚔️ Weekly battle',
  'wb.desc': '{a}  VS  {b}\n\nPick a side — you score points for merging\nyour team\'s creatures. Gems at the end of\nthe week. You can\'t switch sides!',
  'wb.join': 'Team {name}!',
  'wb.mine': '⭐ Your team',
  'wb.points': 'Your points: {n} ⚔️',
  'wb.invite': '📣 Invite friends to my team',
  'wb.share': 'I\'m repping team "{name}" in the Battle of the Week in Brainrot Lab: Merge ({n} points) ⚔️ Whose side are you on?',
  'wb.over': '⚔️ Battle of the Week is over: +{gems}💎 for {points} points!',

  'rename.title': '⭐ LEGENDARY!',
  'rename.desc': 'You raised "{name}"!\nA creature like this deserves ITS OWN name.\nIt stays in your Mempedia forever.',
  'rename.btn': '✏️ Give it a name',
  'rename.prompt': 'Name your legendary:',
  'rename.done': 'Now it\'s "{name}"!',
  'rename.brag': '📣 Brag about your legendary',
  'rename.share': 'My legendary "{name}" is grinding in Brainrot Lab: Merge 🏆 Show me yours!',

  's67.title': '⁉️ 6 7 !!!',
  's67.desc': 'You pulled the SECRET 67!\nThe chance is only 6.7%.\n\nAlmost nobody gets this.\nMerge it up to LEGENDO 67 and become a legend.',
  's67.brag': '📣 Copy the brag',
  's67.share': 'I pulled the secret 67 in Brainrot Lab: Merge — a 6.7% chance 🔵6️⃣7️⃣ Bet you can\'t!',

  'starter.title': '🎁 Newcomer gift',
  'starter.desc': 'Starter pack — ×5 value\n\n150💎 + 5000🪙\n+ 7 days without ads\n\nOne time only!',
  'starter.btn': 'Claim at a discount',

  'offline.title': '💤 While you were away…',
  'offline.desc': 'Your brainrots earned:\n🪙 {n}',
  'offline.claim': 'Claim {n}🪙',
  'offline.claim2': '🎬 Claim ×2 ({n}🪙)',
  'offline.tier': 'Storage holds {hours}h and pays {percent}% of the rate',
  'offline.upsell': '📦 Upgrade storage to {hours}h and 100%',
  'p.offline_vip.title': 'Brainrot storage',
  'p.offline_vip.desc': 'Offline banks 5 hours at the full rate (instead of 2 hours at half) — forever',

  'shop.title': '💎 Shop',
  'shop.freeChest': '🎁 Free chest for an ad',
  'shop.freeChestWait': '🎁 Free chest in {n} min',
  'shop.chest': '📦 Creature chest — {n}💎',
  'shop.failed': 'Purchase did not go through',
  'shop.boost': '⚡ Income ×{mult} (10 min) — {gems}💎',
  'p.starter.title': 'Starter pack — ×5 value',
  'p.starter.desc': '150💎 + 5000🪙 + 7 days without ads (one time)',
  'p.gems_s.title': 'Handful of gems',
  'p.gems_s.desc': '80💎',
  'p.gems_m.title': 'Bag of gems',
  'p.gems_m.desc': '500💎 · +25% value',
  'p.gems_l.title': 'Chest of gems',
  'p.gems_l.desc': '1200💎 · +50% value',
  'p.no_ads.title': 'Remove ads',
  'p.no_ads.desc': 'Removes all forced ads forever',
  'chest.secret': '⁉️ THE SECRET 67 DROPPED!!!',
  'chest.secretFull': 'Board is full — +67💎 (that\'s a sign)',
  'chest.new': 'A new creature from the chest!',
  'chest.full': 'Board is full — +{n}🪙',

  'rarity.common': 'common',
  'rarity.rare': 'rare',
  'rarity.epic': 'epic',
  'rarity.legendary': 'legendary',

  'set.title': '⚙️ Settings',
  'set.soundOn': '🔊 Sound: on',
  'set.soundOff': '🔇 Sound: off',
  'set.lang': '🌐 Language',
  'set.reset': '🗑️ Reset progress',
  'set.resetAsk': 'Delete ALL progress for real?\nThis cannot be undone.',
  'set.resetYes': 'Yes, delete everything',
  'set.version': 'Brainrot Lab: Merge · v{v}',
  'set.credits': 'Made for Yandex Games',
  'set.hapticsOn': '📳 Vibration: on',
  'set.hapticsOff': '📴 Vibration: off',
  'set.shortcut': '📌 Add shortcut to home screen',
  'set.promo': '🎟️ Enter promo code',
  'promo.prompt': 'Enter a promo code (find them on the game\'s socials):',
  'promo.ok': '🎟️ Code accepted!{gems}{egg}',
  'promo.gems': ' +{n}💎',
  'promo.egg': ' + {name}',
  'promo.used': 'This code has already been used',
  'promo.bad': 'Code not found or expired',

  'ach.title': '🏅 Awards {done}/{total}',
  'ach.tab': '🏅 Awards',
  'ach.tabPedia': '📖 Creatures',
  'ach.claimed': 'Claimed',
  'ach.merges': 'Merges: {n}',
  'ach.taps': 'Taps: {n}',
  'ach.spawns': 'Creatures obtained: {n}',
  'ach.golden': 'Golden ones caught: {n}',
  'ach.wins': 'Arena wins: {n}',
  'ach.cups': 'Cups earned: {n}',
  'ach.eggs': 'Eggs hatched: {n}',
  'ach.sold': '"Friends" sold: {n}',
  'ach.legends': 'Legendaries raised: {n}',
  'ach.pedia': 'Mempedia entries: {n}',
  'ach.zones': 'Zones unlocked: {n}',
  'ach.secret': '6️⃣7️⃣ FOREVER',
  'ach.spaghetti': '🍝 Spaghetti tycoon',
  'ach.share': '📣 Brag about it',
  'ach.shareSold': 'I sold {n} "friends" in Brainrot Lab: Merge 💰 Business is business. Find it on Yandex Games!',
  'ach.shareSecret': 'I own the secret 67 in Brainrot Lab: Merge 🔵6️⃣7️⃣ A 6.7% chance. Can you match that?',
  'ach.shareSpaghetti': 'I raised Spaghettissimo Prime in Brainrot Lab: Merge 🍝 Spaghetti tycoon, that\'s me. On Yandex Games!',

  'pass.title': '📔 Journal · tier {tier}/{total}',
  'pass.tab': '📔 Journal',
  'pass.tabQuests': '📋 Daily quests',
  'pass.progress': '{points}/{need} points to the next tier',
  'pass.season': 'Season {id} · {days}d left',
  'pass.how': 'Points: merge +{merge}, win +{win},\nquest +{quest}, golden +{golden}',
  'pass.tier': 'Tier {n}',
  'pass.buy': '🧪 Premium track — {gems}💎',
  'pass.bought': '🧪 Premium active: rewards ×2',
  'pass.premiumOnly': 'premium only',
  'pass.locked': '{n} points to go',
  'pass.coinsMin': '{n} min of income',
  'pass.done': 'Track complete! A new one comes with the new season',
  'pass.buyAsk': 'Premium track, season {id}\n\n• every track reward ×2\n• the "Lab Tech" frame at the end\n\nSpend {gems}💎?',
  'pass.gained': '📔 +{n}',
  'frame.pass': 'Lab Tech',

  'myth.title': '🧪 Mythics {found}/{total}',
  'myth.tab': '🧪 Mythics',
  'myth.fuse': '🧪 Fuse with a legendary',
  'myth.pickTitle': '🧪 Pick the second legendary',
  'myth.pickEmpty': 'You need a second legendary on the board.\nA mythic comes from TWO different ones.',
  'myth.nope': 'That pair does nothing — both creatures are safe',
  'myth.born': '🧪 MYTHIC!',
  'myth.desc': 'Two legendaries made a creature\nnobody around you has.\nIncome of three and a half legendaries.',
  'myth.brag': '📣 Brag about the mythic',
  'myth.share': 'I fused the mythic "{name}" in Brainrot Lab: Merge 🧪 Two legendaries in one. Find it on Yandex Games!',
  'myth.locked': 'Recipe not discovered',
  'myth.hint': 'Fuse these two legendaries',
  'myth.crocaccino': 'Crocaccino Grande',
  'myth.tralaccino': 'Tralaccino Prima',
  'myth.discotambo': 'Disco Tambo Miaussimo',
  'myth.roboletto': 'Roboletto Fruttissimo',
  'myth.guardiano67': 'Guardiano 67',
  'myth.cosmocapy': 'Cosmo Capibarro UFOnissimo',
  'myth.pastalien': 'Pastalieno Galattico',
  'myth.sixseventissimo': 'Sixseventissimo Pasta',

  'pact.btn': '🤝 Friend code',
  'pact.title': '🤝 Friend pact',
  'pact.mine': 'Your code',
  'pact.mineHint': 'Share your code: the reward goes to\nwhoever enters it. To get yours —\nenter a friend\'s code.',
  'pact.copy': '📣 Copy the invite',
  'pact.share': 'I play Brainrot Lab: Merge 🧪 Enter my code {code} and get {gems}💎 plus a rare egg. Find it on Yandex Games!',
  'pact.enter': '🤝 Enter a friend\'s code',
  'pact.prompt': 'Friend\'s code — 4 letters and 2 digits:',
  'pact.namePrompt': 'What\'s your friend\'s name? (optional)',
  'pact.done': '🤝 Pact sealed!',
  'pact.reward': '+{gems}💎, a rare egg\nand +{pct}% income forever',
  'pact.already': 'You already have a pact — one per player',
  'pact.self': 'That\'s your own code 🙂',
  'pact.bad': 'No such code — check the letters and digits',
  'pact.active': '🤝 Pact with {name}',
  'pact.bonus': 'Income +{pct}% forever',
  'pact.partner': 'a friend',
  'pact.honest': 'Without a server the game cannot know what\nyour friend does, so it shows no made-up\nnumbers: only your reward and your bonus.',

  'pres.btn': '🔄 New laboratory',
  'pres.title': '🔄 New laboratory',
  'pres.neurons': '🧠 Neurons: {n}',
  'pres.gain': 'Moving will give you: +{n} 🧠',
  'pres.locked': 'Moving unlocks once every zone is\nopen and the Space Base is filled\nwith top-level creatures.',
  'pres.move': '🚚 Move out ({n} 🧠)',
  'pres.count': 'Moves: {n}',
  'pres.askTitle': '🚚 Ready to move?',
  'pres.keep': 'KEPT:\n💎 gems and purchases\n📖 Mempedia and legendary names\n🏆 cups, leagues, seasons, frames\n🏅 awards and the journal\n🥚 the egg in the incubator\n⚔️ YOUR ARENA TEAM and barracks',
  'pres.lose': 'RESET:\n🪙 coins\n🧪 every board and zone (except the Lab)\n🎁 creature price (starts over)',
  'pres.yes': '🚚 Let\'s move!',
  'pres.done': '🔄 The new laboratory is ready! +{n} 🧠',
  'perk.cooldown': '⏱️ Fast test tubes',
  'perk.cooldownDesc': 'Free creature {n}s sooner',
  'perk.quality': '🧫 Tube quality',
  'perk.qualityDesc': '{n}% of creatures arrive at level 2',
  'perk.lucky': '✨ Lucky merge',
  'perk.luckyDesc': '{n}% of merges jump two levels',
  'perk.income': '💰 Income',
  'perk.incomeDesc': 'Field income +{n}%',
  'perk.offline': '💤 Storage',
  'perk.offlineDesc': 'Offline stores {n} min longer',
  'perk.combo': '👆 Combo',
  'perk.comboDesc': 'Max tap combo ×{n}',
  'perk.buy': '{n} 🧠',
  'perk.max': 'max',

  'skin.title': '🎨 Board look',
  'skin.btn': '🎨 Board look',
  'skin.buy': 'Buy — {gems}💎',
  'skin.on': '✅ Equipped',
  'skin.use': 'Equip',
  'skin.earn': '👹 For 3 bosses defeated',
  'skin.default': 'Laboratory',
  'skin.neon': 'Neon',
  'skin.candy': 'Candy',
  'skin.gold': 'Gold',
  'skin.lab': 'Bio-lab',

  'tour.chip': '🏟️ Weekend cup: {n} wins',
  'tour.title': '🏟️ Weekend cup',
  'tour.desc': 'Arena wins on Saturday and Sunday\ncount toward the cup. Your score: {n}',
  'tour.line': '{wins} wins — {reward}',
  'tour.none': 'The cup only runs on weekends.\nCome back on Saturday and Sunday!',
  'tour.coinsMin': '{n} min of income',

  'boss.btn': '👹 League boss: {name}',
  'boss.done': '👹 League boss defeated ✅',
  'boss.locked': '👹 Boss unlocks in {name} league',
  'boss.title': '👹 {name}',
  'boss.desc': 'One against your five.\nHP {hp} · {dmg} damage every {spd}s · {type}',
  'boss.hint': 'Splash fighters deal only 0.45× to a\nsingle target — bring single-target\nfighters and upgrade the barracks.',
  'boss.fight': '⚔️ Challenge the boss',
  'boss.win': '🏆 BOSS DEFEATED!',
  'boss.reward': 'Reward: +{gems}💎 and a golden egg',
  'boss.lose': '💀 The boss was stronger',
  'boss.loseInfo': 'A boss is a strength check, not luck.\nUpgrade the barracks, merge higher,\nor swap out splash fighters, then return.',
  'boss.rematch': '🎬 Rematch with a ×{buff} buff',
  'ach.bosses': 'Bosses defeated: {n}',

  'season.title': '🗓️ Season results',
  'season.peak': 'Your best league this season:',
  'season.reward': 'Reward: +{gems}💎',
  'season.frame': '🖼️ Profile frame "{name}" unlocked!',
  'season.reset': 'New season cups: {cups}🏆\n(reward tiers are kept — they\ncount your all-time best)',
  'season.claim': 'Claim and start the season',
  'season.now': 'Season {id} · {days}d left',
  'season.frameHint': 'Tap to switch frame',

  'mut.banner': '🧬 {name} is mutating today — income ×{mult}!',
  'mut.card': '🧬 Mutation of the day: income ×{mult}',
  'streak.tomorrow': 'Tomorrow: {reward}',
  'streak.comeBack': 'Come back tomorrow for {reward}!',

  'egg.title': '🥚 Incubator',
  'egg.chipNone': '🥚 empty',
  'egg.chipReady': '🥚 ready!',
  'egg.none': 'The incubator is empty.\n\nEggs come from:\n🏆 every {n}th arena win — common\n⬆️ a new league — rare\n🎉 event rewards — golden',
  'egg.hatchIn': 'Hatches in {time}',
  'egg.ready': 'The egg is ready to hatch!',
  'egg.hatch': '🐣 Hatch it!',
  'egg.ad': '🎬 Speed up by {n} min',
  'egg.adCap': '🎬 Ads: daily limit reached',
  'egg.gem': '💎 −{min} min for {cost}💎',
  'egg.queue': '{n} more 🥚 in queue',
  'egg.got': '🥚 Got a {name}!',
  'egg.gotQueued': '🥚 {name} — queued in the incubator',
  'egg.hatched': '🐣 {name} hatched from the egg!',
  'egg.hatchedFull': 'Board is full — +{n}🪙 instead',
  'egg.gems': '+{n}💎 from the egg',
  'egg.common': 'common egg',
  'egg.rare': 'rare egg',
  'egg.gold': 'golden egg',
  'egg.reward': 'Inside: a level {lvl} creature{gems}',
  'egg.rewardGems': ' + {n}💎',

  'ask.reviewTitle': '⭐ Enjoying the game?',
  'ask.reviewDesc': 'Ratings help the game get featured\non Yandex Games —\nand we make more creatures!',
  'ask.reviewYes': '⭐ Rate the game',
  'ask.later': 'Not now',
  'ask.reviewDone': 'Thank you! It really helps 💛',
  'ask.shortcutTitle': '📌 Home screen shortcut',
  'ask.shortcutDesc': 'Add an icon to your home screen —\none tap to play instead of\nsearching for the game every time.',
  'ask.shortcutYes': '📌 Add shortcut',
  'ask.shortcutDone': 'Done! Look for the icon 📱',
  'ask.authTitle': '☁️ Don\'t lose your progress',
  'ask.authDesc': 'Right now your progress lives only\nin this browser. Log in\nand it will be saved to the cloud.',
  'ask.authYes': '☁️ Log in',
  'ask.authDone': 'Done — progress is in the cloud ☁️',
};

const TR: Record<string, string> = {
  'common.close': 'Kapat',
  'common.claim': 'Al',
  'common.notEnoughCoins': 'Yeterli altın yok!',
  'common.notEnoughGems': 'Yeterli 💎 yok',
  'common.notEnoughFunds': 'Yeterli kaynak yok',
  'common.boardFull': 'Alan dolu!',
  'common.copied': 'Kopyalandı — sohbete at!',
  'common.yes': 'Evet',
  'common.cancel': 'İptal',

  'hud.creature': 'Yaratık ({cost}🪙)',
  'hud.spawnFree': '🎁 Bedava yaratık!',
  'hud.freeIn': '🎁 {n} sn sonra bedava',
  'hud.freeSoon': 'Yeterli altın yok — {n} sn sonra bedava',
  'hud.incomeAd': '🎬 Gelir ×{mult} (2 dk)',
  'hud.boostOn': 'Gelir ×{mult}!',
  'hud.sec': 'sn',
  'hud.min': 'dk',
  'hud.day': 'g',
  'hud.shop': '💎 Mağaza',
  'hud.quests': '📋 Görevler',
  'hud.pedia': '📖 Mempedi',
  'hud.arena': '🏆 Arena',
  'nav.shop': 'Mağaza',
  'nav.quests': 'Görevler',
  'nav.pedia': 'Mempedi',
  'nav.settings': 'Diğer',
  'nav.arena': 'ARENA',
  'golden.tap': '✨ ALTIN! +{n}🪙',

  'pedia.title': '📖 Mempedi {found}/{total}',
  'pedia.discovered': '📖 Keşfedildi: {name}! +{coins}🪙{gems}',
  'pedia.locked': 'Henüz keşfedilmedi',
  'pedia.secretHint': 'Bir sır… sandıklara bak 👀',

  'ftue.drag': '👆 Bir yaratığı diğerinin üzerine sürükle!',
  'ftue.afterMerge': 'Harika! Seviyeler böyle yükselir — gelir de onunla',
  'ftue.income': 'Yaratıklar kendi başına altın kazandırır — sayacı izle 💰',
  'ftue.tap': 'Alandaki yaratıklara dokun — anında ödeme yaparlar!',
  'ftue.arena': 'Arena\'yı aç — takım kur ve kupa için savaş 🏆',
  'event.points': '{emoji} +{n} etkinlik puanı',

  'row.title': '🔒 Ek sıra',
  'row.desc': 'Daha çok yer, daha çok yaratık\nve daha uzun zincirler!',
  'row.buyCoins': '{n}🪙 ile aç',
  'row.buyGems': '{n}💎 ile aç',

  'streak.title': '📅 Günlük ödül',
  'streak.claimDay': '{n}. günün ödülünü al!',
  'streak.lost': '{n} günlük serin bozuldu!',
  'streak.save': '🎬 Reklamla seriyi kurtar',
  'streak.restart': 'Yeni seriye başla',

  'quests.title': '📋 Günün görevleri',
  'quests.line': '{label}\n{prog}/{target} · ödül {coins}🪙 +{gems}💎',
  'quests.footer': 'Her gün yeni görevler!',
  'card.level': 'seviye {n}',
  'card.income': '💰 Her 5 sn\'de {n}🪙 kazandırır',
  'card.type.melee': 'Yakın dövüş — sıranın önündekine vurur',
  'card.type.sniper': 'Keskin nişancı — en yaralıyı bitirir',
  'card.type.splash': 'Alan hasarı — hepsine birden vurur',
  'card.toTeam': '🏆 Arena takımına ekle',
  'card.teamFull': 'Takımda zaten 5 savaşçı var',
  'card.sell': '💰 {n}🪙 karşılığında sat',
  'card.sellSure': '⚠️ Efsanen satılsın mı?',
  'card.sellLegend': '{name} uzun sürede toplandı.\n{n}🪙 karşılığında satılsın mı?',
  'league.wood': 'Tahta',
  'league.bronze': 'Bronz',
  'league.silver': 'Gümüş',
  'league.gold': 'Altın',
  'league.platinum': 'Platin',
  'league.legend': 'EFSANE 67',
  'arena.leagueUp': '⬆️ Yeni lig: {name}!',
  'arena.allClaimed': 'Tüm kupa ödülleri alındı.\nYeni eşikler güncellemede gelecek!',
  'ftue.card': 'Bir yaratığa basılı tut — özellikler, satış ve savaşa gönderme',
  'quest.wins': 'Arena savaşı kazan',
  'quest.merges': 'Yaratık birleştir',
  'quest.spawns': 'Yeni yaratık edin',
  'quest.taps': 'Yaratıklara dokun',

  'zones.title': '🗺️ Bölgeler',
  'zones.chains': '{n} yaratık zinciri',
  'zones.unlock': 'Aç: {coins}🪙 veya {gems}💎',
  'zone.lab': 'Laboratuvar',
  'zone.club': 'Neon Kulüp',
  'zone.watch': 'Gece Nöbeti',
  'zone.space': 'Kozmo Üs',

  'event.info': '{days} gün kaldı · Puan: {points} {emoji}\n\nEtkinlik yaratıkları yeni yaratık alırken\nçıkar (%25 şans). Birleştir ve puan topla!',
  'event.footer': 'Etkinlik bitince yaratıkları altına\ndönüşür — hiçbir şey kaybolmaz!',
  'event.chest': '📦 sandık',
  'event.beach26': 'Plaj Sezonu',
  'event.ny26': 'Yılbaşı Partisi',

  'arena.title': '🏆 Arena · {n}🏆',
  'arena.teamHint': 'Takım (savaşçılar alanda gelir getirmez):',
  'arena.power': 'Takım gücü: {n}',
  'arena.fight': '⚔️ SAVAŞA!',
  'arena.needTeam': 'Önce savaşçı ekle!',
  'arena.atk': '⚔ Saldırı sv.{n}\n{cost}🪙',
  'arena.hp': '❤ Zırh sv.{n}\n{cost}🪙',
  'arena.top': '🏅 En iyi oyuncular',
  'arena.pickTitle': '➕ Alandan savaşçı seç',
  'arena.pickEmpty': 'Alan boş!\nYaratık al ve geri gel.',
  'arena.noRoom': 'Alanda boş yer yok!',
  'arena.win': '🎉 KAZANDIN!',
  'arena.lose': '💀 Kaybettin…',
  'arena.winInfo': '{d}🏆  ·  +{coins}🪙',
  'arena.chestHint': '🎁 Her 3. galibiyette sandık!',
  'arena.loseInfo': '{d}🏆\n\nKışlada takımını güçlendir ya da\ndaha yüksek seviye yaratıklar birleştir.',
  'arena.again': '⚔️ Bir savaş daha!',
  'arena.rematch': '🎬 ×{buff} güçle rövanş',
  'arena.lbTitle': '🏅 Kupa sıralaması',
  'arena.you': 'Sen',
  'arena.player': 'Oyuncu',
  'battle.vs': '⚔️ Sen  VS  {name}',
  'battle.speed': '▶ ×2',

  'wb.title': '⚔️ Haftanın Savaşı',
  'wb.short': '⚔️ Haftalık savaş',
  'wb.desc': '{a}  VS  {b}\n\nBir taraf seç — takımının yaratıklarını\nbirleştirdikçe puan kazanırsın. Hafta sonunda\nkristal. Taraf değiştirilemez!',
  'wb.join': '{name} tarafı!',
  'wb.mine': '⭐ Senin takımın',
  'wb.points': 'Puanın: {n} ⚔️',
  'wb.invite': '📣 Arkadaşlarını takımıma çağır',
  'wb.share': 'Brainrot Lab: Merge\'de Haftanın Savaşı\'nda «{name}» takımındayım ({n} puan) ⚔️ Sen kimin tarafındasın?',
  'wb.over': '⚔️ Haftanın Savaşı bitti: {points} puan için +{gems}💎!',

  'rename.title': '⭐ EFSANE!',
  'rename.desc': '«{name}» yarattın!\nBöyle bir yaratık KENDİ adını hak ediyor.\nMempedi\'nde sonsuza kadar kalır.',
  'rename.btn': '✏️ İsim ver',
  'rename.prompt': 'Efsanenin adı:',
  'rename.done': 'Artık adı «{name}»!',
  'rename.brag': '📣 Efsanenle övün',
  'rename.share': 'Efsanem «{name}» Brainrot Lab: Merge\'de kasıyor 🏆 Sen de göster!',

  's67.title': '⁉️ 6 7 !!!',
  's67.desc': 'GİZLİ 67\'yi yakaladın!\nŞans sadece %6,7.\n\nBunu çok az kişi görür.\nLEGENDO 67\'ye kadar birleştir, efsane ol.',
  's67.brag': '📣 Övünmeyi kopyala',
  's67.share': 'Brainrot Lab: Merge\'de gizli 67\'yi yakaladım — şans sadece %6,7 🔵6️⃣7️⃣ Sen yapabilir misin?',

  'starter.title': '🎁 Yeni oyuncu hediyesi',
  'starter.desc': 'Başlangıç paketi — ×5 değer\n\n150💎 + 5000🪙\n+ 7 gün reklamsız\n\nSadece bir kez!',
  'starter.btn': 'İndirimli al',

  'offline.title': '💤 Sen yokken…',
  'offline.desc': 'Brainrot\'ların kazandı:\n🪙 {n}',
  'offline.claim': '{n}🪙 al',
  'offline.claim2': '🎬 ×2 al ({n}🪙)',
  'offline.tier': 'Depo {hours} saat biriktirir ve oranın %{percent}\'ini öder',
  'offline.upsell': '📦 Depoyu {hours} saate ve %100\'e yükselt',
  'p.offline_vip.title': 'Brainrot deposu',
  'p.offline_vip.desc': 'Çevrimdışı 5 saat tam oranla birikir (2 saat ve yarım oran yerine) — kalıcı',

  'shop.title': '💎 Mağaza',
  'shop.freeChest': '🎁 Reklamla bedava sandık',
  'shop.freeChestWait': '🎁 Bedava sandık {n} dk sonra',
  'shop.chest': '📦 Yaratık sandığı — {n}💎',
  'shop.failed': 'Satın alma tamamlanmadı',
  'shop.boost': '⚡ Gelir ×{mult} (10 dk) — {gems}💎',
  'p.starter.title': 'Başlangıç paketi — ×5 değer',
  'p.starter.desc': '150💎 + 5000🪙 + 7 gün reklamsız (sadece 1 kez)',
  'p.gems_s.title': 'Bir tutam kristal',
  'p.gems_s.desc': '80💎',
  'p.gems_m.title': 'Kristal torbası',
  'p.gems_m.desc': '500💎 · +%25 değer',
  'p.gems_l.title': 'Kristal sandığı',
  'p.gems_l.desc': '1200💎 · +%50 değer',
  'p.no_ads.title': 'Reklamları kapat',
  'p.no_ads.desc': 'Tüm zorunlu reklamları kalıcı olarak kaldırır',
  'chest.secret': '⁉️ GİZLİ 67 ÇIKTI!!!',
  'chest.secretFull': 'Alan dolu — +67💎 (bu bir işaret)',
  'chest.new': 'Sandıktan yeni yaratık!',
  'chest.full': 'Alan dolu — +{n}🪙',

  'rarity.common': 'normal',
  'rarity.rare': 'nadir',
  'rarity.epic': 'epik',
  'rarity.legendary': 'efsanevi',

  'set.title': '⚙️ Ayarlar',
  'set.soundOn': '🔊 Ses: açık',
  'set.soundOff': '🔇 Ses: kapalı',
  'set.lang': '🌐 Dil',
  'set.reset': '🗑️ İlerlemeyi sıfırla',
  'set.resetAsk': 'TÜM ilerleme silinsin mi?\nBu geri alınamaz.',
  'set.resetYes': 'Evet, hepsini sil',
  'set.hapticsOn': '📳 Titreşim: açık',
  'set.hapticsOff': '📴 Titreşim: kapalı',
  'set.shortcut': '📌 Ana ekrana kısayol ekle',
  'set.promo': '🎟️ Promo kodu gir',
  'promo.prompt': 'Promo kodu gir (oyunun sosyal medyasında ara):',
  'promo.ok': '🎟️ Kod kabul edildi!{gems}{egg}',
  'promo.gems': ' +{n}💎',
  'promo.egg': ' + {name}',
  'promo.used': 'Bu kod zaten kullanıldı',
  'promo.bad': 'Kod bulunamadı veya süresi doldu',

  'ach.title': '🏅 Ödüller {done}/{total}',
  'ach.tab': '🏅 Ödüller',
  'ach.tabPedia': '📖 Yaratıklar',
  'ach.claimed': 'Alındı',
  'ach.merges': 'Birleştirme: {n}',
  'ach.taps': 'Dokunma: {n}',
  'ach.spawns': 'Elde edilen yaratık: {n}',
  'ach.golden': 'Yakalanan altın: {n}',
  'ach.wins': 'Arena zaferi: {n}',
  'ach.cups': 'Toplanan kupa: {n}',
  'ach.eggs': 'Çıkan yumurta: {n}',
  'ach.sold': 'Satılan «dost»: {n}',
  'ach.legends': 'Yetiştirilen efsane: {n}',
  'ach.pedia': 'Mempedia kaydı: {n}',
  'ach.zones': 'Açılan bölge: {n}',
  'ach.secret': '6️⃣7️⃣ SONSUZA DEK',
  'ach.spaghetti': '🍝 Spagetti kralı',
  'ach.share': '📣 Övün',
  'ach.shareSold': 'Brainrot Lab: Merge oyununda {n} «dost» sattım 💰 İş iştir. Yandex Games\'te!',
  'ach.shareSecret': 'Brainrot Lab: Merge oyununda gizli 67 bende 🔵6️⃣7️⃣ Şans %6,7. Yapabilir misin?',
  'ach.shareSpaghetti': 'Brainrot Lab: Merge oyununda Spaghettissimo Prime yetiştirdim 🍝 Spagetti kralı benim. Yandex Games\'te!',

  'pass.title': '📔 Günlük · kademe {tier}/{total}',
  'pass.tab': '📔 Günlük',
  'pass.tabQuests': '📋 Günlük görevler',
  'pass.progress': 'Sonraki kademeye {points}/{need} puan',
  'pass.season': 'Sezon {id} · {days}g kaldı',
  'pass.how': 'Puan: birleştirme +{merge}, zafer +{win},\ngörev +{quest}, altın +{golden}',
  'pass.tier': '{n}. kademe',
  'pass.buy': '🧪 Premium hat — {gems}💎',
  'pass.bought': '🧪 Premium aktif: ödüller ×2',
  'pass.premiumOnly': 'sadece premium',
  'pass.locked': '{n} puan kaldı',
  'pass.coinsMin': '{n} dk gelir',
  'pass.done': 'Hat tamamlandı! Yenisi yeni sezonla gelir',
  'pass.buyAsk': '{id} sezonu premium hattı\n\n• tüm hat ödülleri ×2\n• finalde «Laborant» çerçevesi\n\n{gems}💎 harcanacak?',
  'pass.gained': '📔 +{n}',
  'frame.pass': 'Laborant',

  'myth.title': '🧪 Mitikler {found}/{total}',
  'myth.tab': '🧪 Mitikler',
  'myth.fuse': '🧪 Bir efsaneyle birleştir',
  'myth.pickTitle': '🧪 İkinci efsaneyi seç',
  'myth.pickEmpty': 'Alanda ikinci bir efsane gerekli.\nMitik İKİ farklı efsaneden çıkar.',
  'myth.nope': 'Bu çift bir şey vermez — yaratıklar sağlam',
  'myth.born': '🧪 MİTİK!',
  'myth.desc': 'İki efsane, etrafta kimsede olmayan\nbir yaratık doğurdu.\nGeliri üç buçuk efsane kadar.',
  'myth.brag': '📣 Mitikle övün',
  'myth.share': 'Brainrot Lab: Merge oyununda «{name}» mitiğini birleştirdim 🧪 Bir yaratıkta iki efsane. Yandex Games\'te!',
  'myth.locked': 'Tarif keşfedilmedi',
  'myth.hint': 'Bu iki efsaneyi birleştir',
  'myth.crocaccino': 'Crocaccino Grande',
  'myth.tralaccino': 'Tralaccino Prima',
  'myth.discotambo': 'Disco Tambo Miaussimo',
  'myth.roboletto': 'Roboletto Fruttissimo',
  'myth.guardiano67': 'Guardiano 67',
  'myth.cosmocapy': 'Cosmo Capibarro UFOnissimo',
  'myth.pastalien': 'Pastalieno Galattico',
  'myth.sixseventissimo': 'Sixseventissimo Pasta',

  'pact.btn': '🤝 Arkadaş kodu',
  'pact.title': '🤝 Arkadaş paktı',
  'pact.mine': 'Kodun',
  'pact.mineHint': 'Kodunu paylaş: ödülü kodu giren alır.\nKendi ödülünü almak için\narkadaşının kodunu gir.',
  'pact.copy': '📣 Daveti kopyala',
  'pact.share': 'Brainrot Lab: Merge oynuyorum 🧪 {code} kodumu gir, {gems}💎 ve nadir yumurta kazan. Yandex Games\'te!',
  'pact.enter': '🤝 Arkadaş kodu gir',
  'pact.prompt': 'Arkadaşının kodu — 4 harf ve 2 rakam:',
  'pact.namePrompt': 'Arkadaşının adı? (atlanabilir)',
  'pact.done': '🤝 Pakt kuruldu!',
  'pact.reward': '+{gems}💎, nadir yumurta\nve kalıcı +%{pct} gelir',
  'pact.already': 'Paktın zaten var — oyuncu başına bir tane',
  'pact.self': 'Bu senin kendi kodun 🙂',
  'pact.bad': 'Böyle bir kod yok — harfleri ve rakamları kontrol et',
  'pact.active': '🤝 {name} ile pakt',
  'pact.bonus': 'Kalıcı +%{pct} gelir',
  'pact.partner': 'arkadaşın',
  'pact.honest': 'Sunucu olmadan oyun arkadaşının ne yaptığını\nbilemez, bu yüzden uydurma sayı göstermez:\nburada sadece kendi ödülün ve bonusun var.',

  'pres.btn': '🔄 Yeni laboratuvar',
  'pres.title': '🔄 Yeni laboratuvar',
  'pres.neurons': '🧠 Nöron: {n}',
  'pres.gain': 'Taşınma sana verecek: +{n} 🧠',
  'pres.locked': 'Taşınma, tüm bölgeler açıldığında ve\nKozmo Üs en yüksek seviye\nyaratıklarla dolduğunda açılır.',
  'pres.move': '🚚 Taşın ({n} 🧠)',
  'pres.count': 'Taşınma: {n}',
  'pres.askTitle': '🚚 Taşınalım mı?',
  'pres.keep': 'KORUNUR:\n💎 kristaller ve satın alımlar\n📖 Mempedia ve efsane isimleri\n🏆 kupalar, ligler, sezonlar, çerçeveler\n🏅 ödüller ve günlük\n🥚 kuluçkadaki yumurta\n⚔️ ARENA TAKIMIN ve kışla',
  'pres.lose': 'SIFIRLANIR:\n🪙 altınlar\n🧪 tüm alanlar ve bölgeler (Laboratuvar hariç)\n🎁 yaratık fiyatı (baştan başlar)',
  'pres.yes': '🚚 Taşınıyoruz!',
  'pres.done': '🔄 Yeni laboratuvar hazır! +{n} 🧠',
  'perk.cooldown': '⏱️ Hızlı tüpler',
  'perk.cooldownDesc': 'Ücretsiz yaratık {n}s daha erken',
  'perk.quality': '🧫 Tüp kalitesi',
  'perk.qualityDesc': 'Yaratıkların %{n}\'i 2. seviye gelir',
  'perk.lucky': '✨ Şanslı birleşme',
  'perk.luckyDesc': 'Birleşmelerin %{n}\'i iki seviye atlar',
  'perk.income': '💰 Gelir',
  'perk.incomeDesc': 'Alan geliri +%{n}',
  'perk.offline': '💤 Depo',
  'perk.offlineDesc': 'Çevrimdışı {n} dk daha uzun toplar',
  'perk.combo': '👆 Kombo',
  'perk.comboDesc': 'Maksimum dokunma kombosu ×{n}',
  'perk.buy': '{n} 🧠',
  'perk.max': 'maks.',

  'skin.title': '🎨 Alan görünümü',
  'skin.btn': '🎨 Alan görünümü',
  'skin.buy': 'Satın al — {gems}💎',
  'skin.on': '✅ Takılı',
  'skin.use': 'Tak',
  'skin.earn': '👹 3 patron yenince',
  'skin.default': 'Laboratuvar',
  'skin.neon': 'Neon',
  'skin.candy': 'Şeker',
  'skin.gold': 'Altın',
  'skin.lab': 'Biyo-lab',

  'tour.chip': '🏟️ Hafta sonu kupası: {n} zafer',
  'tour.title': '🏟️ Hafta sonu kupası',
  'tour.desc': 'Cumartesi ve pazar arena zaferleri\nkupaya sayılır. Puanın: {n}',
  'tour.line': '{wins} zafer — {reward}',
  'tour.none': 'Kupa sadece hafta sonu geçerli.\nCumartesi ve pazar uğra!',
  'tour.coinsMin': '{n} dk gelir',

  'boss.btn': '👹 Lig patronu: {name}',
  'boss.done': '👹 Lig patronu yenildi ✅',
  'boss.locked': '👹 Patron {name} liginde açılır',
  'boss.title': '👹 {name}',
  'boss.desc': 'Beşinize karşı bir dev.\nHP {hp} · {spd}s\'de {dmg} hasar · {type}',
  'boss.hint': 'Alan hasarı verenler tek hedefe\n0.45 güçle vurur — patrona tek hedefli\nsavaşçı götür ve kışlayı geliştir.',
  'boss.fight': '⚔️ Patrona meydan oku',
  'boss.win': '🏆 PATRON YENİLDİ!',
  'boss.reward': 'Ödül: +{gems}💎 ve altın yumurta',
  'boss.lose': '💀 Patron daha güçlüydü',
  'boss.loseInfo': 'Patron şans değil, güç sınavıdır.\nKışlayı geliştir, daha yükseğe birleştir\nveya alan hasarlıları değiştir, sonra dön.',
  'boss.rematch': '🎬 ×{buff} güçle rövanş',
  'ach.bosses': 'Yenilen patron: {n}',

  'season.title': '🗓️ Sezon sonuçları',
  'season.peak': 'Bu sezonun en iyi ligin:',
  'season.reward': 'Ödül: +{gems}💎',
  'season.frame': '🖼️ «{name}» profil çerçevesi açıldı!',
  'season.reset': 'Yeni sezon kupaları: {cups}🏆\n(ödül eşikleri korunur — kişisel\nrekoruna göre sayılır)',
  'season.claim': 'Al ve sezona başla',
  'season.now': 'Sezon {id} · {days}g kaldı',
  'season.frameHint': 'Çerçeveyi değiştirmek için dokun',

  'mut.banner': '🧬 Bugün {name} mutasyona uğradı — gelir ×{mult}!',
  'mut.card': '🧬 Günün mutasyonu: gelir ×{mult}',
  'streak.tomorrow': 'Yarın: {reward}',
  'streak.comeBack': 'Yarın {reward} için geri dön!',

  'egg.title': '🥚 Kuluçka',
  'egg.chipNone': '🥚 boş',
  'egg.chipReady': '🥚 hazır!',
  'egg.none': 'Kuluçka boş.\n\nYumurta kaynakları:\n🏆 her {n}. arena zaferi — normal\n⬆️ yeni lig — nadir\n🎉 etkinlik ödülleri — altın',
  'egg.hatchIn': '{time} sonra çıkacak',
  'egg.ready': 'Yumurta çıkmaya hazır!',
  'egg.hatch': '🐣 Çıkar!',
  'egg.ad': '🎬 {n} dk hızlandır',
  'egg.adCap': '🎬 Reklam: günlük limit doldu',
  'egg.gem': '💎 {cost}💎 ile −{min} dk',
  'egg.queue': 'Sırada {n} 🥚 daha var',
  'egg.got': '🥚 {name} kazandın!',
  'egg.gotQueued': '🥚 {name} — kuluçka sırasında',
  'egg.hatched': '🐣 Yumurtadan {name} çıktı!',
  'egg.hatchedFull': 'Alan dolu — yerine +{n}🪙',
  'egg.gems': 'Yumurtadan +{n}💎',
  'egg.common': 'normal yumurta',
  'egg.rare': 'nadir yumurta',
  'egg.gold': 'altın yumurta',
  'egg.reward': 'İçinde: {lvl}. seviye yaratık{gems}',
  'egg.rewardGems': ' + {n}💎',

  'ask.reviewTitle': '⭐ Oyunu sevdin mi?',
  'ask.reviewDesc': 'Puanlar oyunun Yandex Games\nvitrinlerine girmesine yardım eder —\nbiz de daha çok yaratık yaparız!',
  'ask.reviewYes': '⭐ Oyunu puanla',
  'ask.later': 'Şimdi değil',
  'ask.reviewDone': 'Teşekkürler! Gerçekten yardımcı olur 💛',
  'ask.shortcutTitle': '📌 Ana ekran kısayolu',
  'ask.shortcutDesc': 'Telefon ekranına simge ekle —\nher seferinde oyunu aramak yerine\ntek dokunuşla gir.',
  'ask.shortcutYes': '📌 Kısayol ekle',
  'ask.shortcutDone': 'Hazır! Simgeyi ekranda ara 📱',
  'ask.authTitle': '☁️ İlerlemeni kaybetme',
  'ask.authDesc': 'Şu anda ilerlemen sadece\nbu tarayıcıda. Hesabına gir,\nbulutta saklanacak.',
  'ask.authYes': '☁️ Hesaba gir',
  'ask.authDone': 'Hazır — ilerleme bulutta ☁️',
  'set.version': 'Brainrot Lab: Merge · v{v}',
  'set.credits': 'Yandex Games için yapıldı',
};

const DICTS: Record<Lang, Record<string, string>> = { ru: RU, en: EN, tr: TR };

/** Строка по ключу с подстановкой {param}. Фолбэк: язык → RU → сам ключ. */
export function t(key: string, params?: Record<string, string | number>): string {
  let s = DICTS[lang][key] ?? RU[key] ?? key;
  if (params) for (const k in params) s = s.split(`{${k}}`).join(String(params[k]));
  return s;
}

// Латинские имена существ для EN/TR — по id цепочки (и по id события для событийных).
const LATIN: Record<string, string[]> = {
  coffee: ['Granello', 'Tazzina', 'Latterina', 'Ballerino Cappuccino', 'Grande Mocaccino', 'Prima Espressima'],
  croc: ['Uovino', 'Coccodrillino', 'Coccodrino', 'Aviocrocodilo', 'Bombardino', 'Mega Crocodilissimo'],
  shark: ['Pesciolino', 'Squaletto', 'Tralalino', 'Squalo Ballerino', 'Megalodino', 'Grande Tralalissimo'],
  drum: ['Scheggia', 'Tamburello', 'Tung Tungano', 'Tamburando', 'Notte Tamburandissimo', 'Sahurando Legendo'],
  cat: ['Gattino', 'Gatto Beatino', 'Miao DJino', 'Gatto Disco Fonio', 'Mega Miaomix', 'Legendo Gattofono'],
  robot: ['Bulloncino', 'Robo Piccolino', 'Bitbox 3000', 'Robo Ballerino', 'Giga Danzato', 'Primo Robotissimo'],
  fruit: ['Semino', 'Fragolella', 'Bacca Ladina', 'Banano Stiliso', 'Duetto Romantico', 'Frutto Legendario'],
  stick: ['Legnetto', 'Palo Guardino', 'Bastone Notturno', 'Guardiando Bastone', 'Grande Postovissimo', 'Legendo Vigilante'],
  sixseven: ['Sei', 'Sette', 'Sei-Sette', '67 Danzato', 'Mega 67', 'LEGENDO 67'],
  capy: ['Capibarino', 'Capi Chillo', 'Capibarello', 'Cosmo Capibara', 'Capibara Comandante', 'Grande Capibarissimo'],
  ufo: ['Lucino', 'UFOlino', 'Dischetto', 'Ospite Verdino', 'Mega Alienoso', 'Legendo Galattico'],
  noodle: ['Maccherino', 'Nudlino', 'Spaghetti Joe', 'Nudlo Cosmo', 'Giga Pasta', 'Spaghettissimo Prime'],
  beach26: ['Sabbiolino', 'Conchiglino', 'Crabo DJino', 'Palmando Cocosini', 'Grande Solecissimo'],
  ny26: ['Fioccolino', 'Sugrobino', 'Alberello Ballerino', 'Nonno Gelato', 'Grande Salutissimo'],
};

/** Имя существа: в RU — из конфига, в EN/TR — латиница (фолбэк на конфиг). */
export function creatureName(cfg: Chain, level: number): string {
  if (lang === 'ru') return cfg.names[level];
  return LATIN[cfg.id]?.[level] ?? cfg.names[level];
}

export const rarityName = (level: number) =>
  t(level >= 5 ? 'rarity.legendary' : level >= 4 ? 'rarity.epic' : level >= 2 ? 'rarity.rare' : 'rarity.common');

// Ники соперников арены: живые, «как у настоящих игроков».
const NICKS_RU = [
  'Кирилл_2013', 'xX_БравлеР_Xx', 'НагибаторТоля', 'мама сказала можно', 'Скуф67',
  'ПростоДаня', 'КапибараЛюб', 'ЗубастикПро', 'aunt_walera', 'СЛИВКИ_ОБЩЕСТВА',
  'Тимофей TV', 'девочка_вайб', 'КрутойПерец99', 'Их_Бин_Ту', 'сигма-с-урока',
  'ЛещДесантный', 'Полиночка)', 'ГномГномыч', 'Абобус228', 'человек-роблокс',
];
const NICKS_LAT = [
  'Kirill_2013', 'xX_BrawleR_Xx', 'NoScopeToni', 'mom_said_yes', 'BigChungus67',
  'JustDaniel', 'CapybaraLover', 'SharkTeethPro', 'uncle_bob', 'CREAM_OF_SOCIETY',
  'Timo TV', 'girly_vibe', 'CoolPepper99', 'Ich_Bin_Zwei', 'sigma_from_class',
  'ParatrooperFish', 'Ayse_gamer', 'GnomeGnomich', 'Abobus228', 'roblox-person',
];
export const nicks = () => (lang === 'ru' ? NICKS_RU : NICKS_LAT);
