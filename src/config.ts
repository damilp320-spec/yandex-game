...
/**
 * Награда за заказ = доход этого существа за ORDER_MINUTES минут.
 *
 * Была фиксированная таблица [0, 40, 120, 300, 750, 1800], и она ломала смысл сдачи:
 * существо 3-го уровня приносит 192 монеты в минуту, а заказ платил 300 — то есть
 * окупался за полторы минуты, после чего держать существо было выгоднее всегда.
 * Вдобавок награда не зависела от цепочки, хотя доход зависит: существо «Космо-Базы»
 * зарабатывает в 4,85 раза больше кофейного, а платили за него столько же.
 *
 * Теперь выбор честный: сдать — это получить сразу несколько минут дохода и
 * освободить клетку; держать — получать те же монеты медленно, но занимать место.
 * И награда сама растёт с прогрессом, без правки таблиц.
 *
 * Горизонт 3 минуты выбран прогоном sim: при 7 минутах сдавать было выгодно всегда,
 * а монет к 14-му дню накапливалось 63M против сливов на порядок меньше.
 */
export const ORDER_MINUTES = 3;
export const orderReward = (chain: number, level: number) =>
  Math.round((incomeOf(chain, level) * (60_000 / INCOME.periodMs) * ORDER_MINUTES) / 10) * 10;

/** Каждый N-й выполненный заказ даёт сундук — причина сдавать даже когда монет вдоволь. */
export const ORDER_CHEST_EVERY = 12;
